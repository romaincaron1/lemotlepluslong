...
import { createThree, verifyWord } from "../data/dictionnary";
	...
	useEffect(() => {
		if (!fPlayerOnBlackCase && !sPlayerOnBlackCase) {
			...
			if (verifyWord(fValue)) {
				...
				if (fPlayerCase - fNbCases >= 0) {
					setFPlayerCase(fPlayerCase - fNbCases);
				} else {
					setFPlayerCase(18 + (fPlayerCase - fNbCases));
				}
			}
			resetRound();
			alert.show(<div className="alert-bloc">{fSentence}</div>);
		} else if (sPlayerOnBlackCase && !fPlayerOnBlackCase && sValid === 1) {
			var sSentence = (
				<p>
					Le mot entré par{" "}
					<span className="important-word">{secondPlayer.name}</span> n'est pas
			...
			if (verifyWord(sValue)) {
				...
				if (sPlayerCase - sNbCases >= 0) {
					setSPlayerCase(sPlayerCase - sNbCases);
				} else {
					setSPlayerCase(18 + (sPlayerCase - sNbCases));
				}
			}
			resetRound();
